...
class Obstacles{
    ...
    _updateSpheres(matrixWorld){
        ...
        for(let i=0;i<this.meshes.length;i++){
            ...
            const sphere = boundingSphere.clone().applyMatrix4(mesh.matrixWorld).applyMatrix4(inverseMatrix);
            sphere.radius+=6
            this.spheres.push(sphere)
        }
        return this.spheres
    }
    apply(clothSim,clothMesh){
        this._updateSpheres(clothMesh.matrixWorld)
        const particles=clothSim.particles
        const self=this
        const applyObstacle=( p1, obstacle )=> {
            const diff = self._tool.subVectors( p1.position,obstacle.center);
            const currentDist = diff.length();
            if ( currentDist >= obstacle.radius ) return
            const correction = diff.multiplyScalar( (obstacle.radius - currentDist) / currentDist )
            p1.position.add( correction )
        }
        for ( let i = 0; i < particles.length; i ++ ) 
            for(let j=0;j<this.spheres.length;j++)
                applyObstacle( particles[ i ], this.spheres[j] );
    }
}
class ClothSimulate{
    _tool=new THREE.Vector3()
    ...
    applyObstacles(){
        ...
        for ( let i = 0; i < this.particles.length; i ++ ) {
            ...
            applyObstacle( particle, {center:new THREE.Vector3(0,0,0),radius:140*5} );
        }
    }
