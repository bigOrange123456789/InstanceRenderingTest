...
function loadGeometrySperated( speciesLib ) {
	...
	for ( let i = 0; i < pFactor; i ++ ) {
		///////////////////////////////////////////////////////////////////////////////////////////////////////
		let forestLoader = new Worker( "../myJS1_3/forestLoaderWorker.js" );
		...
		forestLoader.onmessage = function ( evt ) {
			if ( evt.data.command == "assemble" ) {
				...
				for ( let sp of evt.data.species ) {
					...
					console.log("finish sp:",sp)
				}
			}
		}
		// var myForestLoader=new ForestLoader(function ( evt ) {
		// 	if ( evt.command == "assemble" ) {
		// 		assembleData( speciesLib, evt[ "data" ] );
		// 	} else if ( evt.command == "finished" ) {
		// 		for ( let sp of evt.species ) {
		// 			window[ "loadingStatus" ][ "geometry" ][ sp ] = "finished";
		// 			console.log("finish sp:",sp)
		// 		}
		// 	}
		// })
		// myForestLoader.postMessage( { command: "loading", data: speciesLibArray[ i ] } );
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////
	}
}
