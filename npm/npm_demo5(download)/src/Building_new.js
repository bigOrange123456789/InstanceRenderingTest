...
export class Building{
    ...
    processMesh(meshNodeList,structList,matrixList){//这个模型已经拆分好了
        for(let i=0; i<meshNodeList.length; i++){//遍历全部mesh对象
            ...
            if(this.meshlist.length==21484)this.merge(800)
        }
    ...
    merge(n_clusters){
        ...
        for(let i=0;i<window.meshlist.length;i++){
            const center=[0,0,0]
            const spheres=window.meshlist[i].spheres
            for(let j=0;j<spheres.length;j++){
                center[0]+=spheres[j].center.x
                center[1]+=spheres[j].center.y
                center[2]+=spheres[j].center.z
            }
        ...
        console.log(dataSet)
        const clusterAssment=kMeans(dataSet, n_clusters)
        ...
        downloadJSON(clusterAssment, 'clusterAssment.json');
