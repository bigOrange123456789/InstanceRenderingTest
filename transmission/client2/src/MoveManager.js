                ...
                if(scope.myMakeOneRoamStep.preview(scope.myPreviewflag,scope.avatar,scope.roamPath)) {
                    ...
                    if(scope.myPreviewflag=== scope.roamPath.length)
                        if(scope.isLoop)scope.myPreviewflag = 0;
                            ...
                            alert(window.root.children.length)
                        }
                }
            } //else scope.myMakeOneRoamStep.firstTime=true;
        ...
        function movetoPos(avatar,scope){//移动
            if(scope.stepIndex<scope.stepIndex_max){
                ...
                if(scope.firstTime){
                    ...
                    dt=1;
                }else if(scope.time===0)dt=1;
                else dt=scope.time/5;
                scope.time=0;

                avatar.position.x+=scope.dx*dt;
                avatar.position.y+=scope.dy*dt;
                avatar.position.z+=scope.dz*dt;
