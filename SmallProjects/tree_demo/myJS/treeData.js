...
/*     let geo= new THREE.SphereBufferGeometry(0.15,4,4);
    let mesh=new THREE.Mesh(geo,material);
    mesh.name="material_test";
    window.editor.addObject(mesh); */
}
function buildInstancedLeaves(leavesInfo,meshes,materials){
    let leavesGroup=new THREE.Group();
    leavesGroup.name="leaves_group;"
    let leavesInstance={};
    let id=0;
    ...
    for(let meshID of Object.keys(leavesInstance)){
       ...
       if(!mat.alphaMap){
            ...
            for(let i=0;i<count;i++){
                ...
                instancedLeaves.setMatrixAt(i,matrix);

            }
            leavesGroup.add(instancedLeaves);
        }
    } 
...
function loadLOD(species,level){
    let material=new THREE.MeshBasicMaterial();
       ...
       loader.load(
        // resource URL
        `../models/forest/${species}/${level}_color.jpeg`,
        ...
        loader.load(
            // resource URL
            `../models/forest/${species}/${level}_alpha.jpeg`,
...
class TreeData{
    constructor(obj){
        let data=obj.data;
        let type=obj.type;
        let species_name=obj.species_name;
        let branchMatID=obj.branchMatID;
        let isLoadLODTexture=obj.isLoadLODTexture;
        this.meshes={};
        this.materials={};
        this.materialsInfo=[];
        this.rootIndices=[];
        ...
        this.meshCount=-1;
        this.lodMaterial={};
        this.branchMatID=branchMatID;
        if(!data)return;

        if(isLoadLODTexture){
            this.lodMaterial["1"]=loadLOD(species_name,1);
            this.lodMaterial["2"]=loadLOD(species_name,2);
            this.lodMaterial["3"]=loadLOD(species_name,3);
            this.lodMaterial["4"]=loadLOD(species_name,4);
           }
     if(type=="xml"){
        let x2js = new X2JS();
        ...
        for(let i=0;i<this.maxLevel+1;i++)this.branchesIdByLayer.push([]);
        for(let branch of Object.values(this.branches)){
            this.branchesIdByLayer[branch.layer].push(branch.id);
        }
         //weld point
    ...
    produceTreeGeo(obj){
        ...
        let style=obj.style;
        let level=obj.level;
        let barkMaterialID=this.branchMatID;
        let useBasicMaterial=obj.useBasicMaterial;
        let lod=obj.lod;
        let  TreeSpeciesObj=obj.TreeSpeciesObj;

        if(TreeSpeciesObj)lod=9;
        //console.log(this);
        let treeGroup = new THREE.Group();
        treeGroup.name="treeGroup";
        let values=Object.values(this.branches);
        let lodBranch=new THREE.Geometry();
        for(let branch of values){
          
            let treeIndividualFullGeo=null;
            let treeIndividualLODGeo=null;
            if(TreeSpeciesObj){
                treeIndividualFullGeo=TreeSpeciesObj.fullMeshes[branch.layer];
                treeIndividualLODGeo=TreeSpeciesObj.LODMeshes[branch.layer];
            }
         if(branch.layer<lod){
            if(branch.spinePoints.R[0]>0.05) {
                branch.produceBranchGeo(
                    {_treeGroup:treeGroup,
                     _style:style,
                     _level:branch.layer,
                     bark_material:this.materials[barkMaterialID],
                     uvFactor:0.2,
                     haveHelper:false,
                    lodFactor:Math.min(4,lod)*0.175+0.3,
                    treeIndividualGeo:treeIndividualFullGeo
                    });
               
            }
            branch.produceLeavesOnBranch(this);
         }
          if(branch.layer==lod||TreeSpeciesObj){
              if(branch.layer==0)continue;
             if(lod==1){
                   let branchPos=new THREE.Vector3(branch.posAbs.x ,branch.posAbs.y,branch.posAbs.z);
                   let sps=   this.branches["1"].spinePoints;
                   let last=sps.length-1;
                    let treeTopPos=new THREE.Vector3(sps.posX[last],sps.posY[last],sps.posZ[last]);
                    let dist=treeTopPos.distanceTo(branchPos);
                    if(dist<0.8)continue;
             }
             let lodPiece=this.produceLODBranch(branch);
             lodBranch.merge(lodPiece);
             if(treeIndividualLODGeo){
                treeIndividualLODGeo.merge(lodPiece);
             }
         }
        ...
        //handle lod
        let lodBufferGeo=new THREE.BufferGeometry().fromGeometry(lodBranch); 
        let currentLODMaterial=this.lodMaterial[""+lod]; 
        let lodMesh=new THREE.Mesh(lodBufferGeo,currentLODMaterial);  

        lodMesh.castShadow=true;
        let waitAlphaMap=setInterval(()=>{
            if(lod>4) clearInterval(waitAlphaMap);
            if(!currentLODMaterial)return; 
              if(currentLODMaterial.alphaMap){
                  clearInterval(waitAlphaMap);
                  lodMesh.customDepthMaterial = new THREE.MeshDepthMaterial( {
                    depthPacking: THREE.RGBADepthPacking,
                    alphaMap: currentLODMaterial.alphaMap,
                    alphaTest: 0.4
                });
              }
        },1000);
        lodMesh.name="lodMesh_"+lod;
        treeGroup.add(lodMesh);
          //draw Leaves
          if(lod==9) {
           let leavesGeo=null;
        let BufferGeometryUtils=null;
           if(TreeSpeciesObj){
            ...
            leavesGeo= TreeSpeciesObj.fullLeavesGeo;
            BufferGeometryUtils=obj.BufferGeoU;
           }
            ...
            for(let meshID of Object.keys(this.leaves)){
                ...
                for(let matID of Object.keys(this.leaves[meshID])){
                    ...
                    for(let i=0;i<count;i++){
                        ...
                        instancedLeaves.setMatrixAt(i,this.leaves[meshID][matID][i]);
                        if(leavesGeo){
                                 let now_geo=new THREE.BufferGeometry();
                                 now_geo.copy(mesh);
                                 now_geo.applyMatrix4(this.leaves[meshID][matID][i]);
                                 leavesGeoArray.push(now_geo);
                        }
                    }
                    if(leavesGeo) leavesGeo[`${matID}`]=BufferGeometryUtils.mergeBufferGeometries(leavesGeoArray);
                    treeGroup.add(instancedLeaves);
                }
    ...
    produceLODBranch(branch){
        let geo=new THREE.PlaneGeometry();
        let mat=new THREE.Matrix4();
        let matO=new THREE.Matrix4();
        matO.makeTranslation(0,0.5,0);
        let matT=new THREE.Matrix4();
        matT.makeTranslation(branch.posAbs.x,branch.posAbs.y,branch.posAbs.z);
        //
        let sp=branch.spinePoints;
        let last=sp.length-1;
        let beginP=new THREE.Vector3(sp.posX[0],sp.posY[0],sp.posZ[0]);
        let lastP=new THREE.Vector3(sp.posX[last],sp.posY[last],sp.posZ[last]);
        lastP.addScaledVector(beginP,-1);
        let matS=new THREE.Matrix4();
        let scaleFactor=lastP.length()*1.1;
        matS.makeScale(scaleFactor,scaleFactor,scaleFactor);
        let matR=new THREE.Matrix4();
        matR=alignAxis(new THREE.Vector3(0,1,0),lastP);
        mat=mat.multiply(matT);
        mat=mat.multiply(matS);
        mat=mat.multiply(matR);
        mat=mat.multiply(matO);
        geo.applyMatrix4(mat);
        return geo;
    }
  producePartialStructureBelow(id,barkMaterialID=null,group,container){
        let target=null;
        let topLevelFlag=false;
        let leavesContainer=null;
        if(this.branchMatID)barkMaterialID=this.branchMatID;
        let geo=null;
        if(!group){
            target=new THREE.Group();
            geo=new THREE.Geometry();
            target.name="sub_true_"+id;
            topLevelFlag=true;
        }else{
            target=group;
            geo=group;
        ...
        if(!container){
            ...
            leavesContainer=container;
        }
 
      let branch=this.branches[id];
      if(!branch)return;
     /*  let post_material=this.materials[barkMaterialID]; */
      let basic_material=new THREE.MeshBasicMaterial();

      let waitMaploading=setInterval(()=>{
          if(!this.materials[barkMaterialID].map)return;
          clearInterval(waitMaploading);
          basic_material.map=this.materials[barkMaterialID].map;
      ...
      branch.produceLeavesOnBranch(this,leavesContainer);
      for(let childID of branch.childrenBranchIndices){
        this.producePartialStructureBelow(childID,barkMaterialID,geo,leavesContainer);
      }
      ...
      if(topLevelFlag){
        ...
        target["materialLib"]=[];
        let post_material=this.materials[barkMaterialID];
        let basic_material=new THREE.MeshBasicMaterial({map:post_material.map});
        target["materialLib"].push(basic_material);
        let waitMaploading=setInterval(()=>{
            if(!this.materials[barkMaterialID].map)return;
            clearInterval(waitMaploading);
            basic_material.map=this.materials[barkMaterialID].map;
            basic_material.needsUpdate=true;
            target["materialLib"][0]["backMap"]=basic_material.map;
        },1000);
      
        let bufferGeo=new THREE.BufferGeometry().fromGeometry(geo);
        let branchMesh=new THREE.Mesh(bufferGeo,basic_material);
        branchMesh.name="branch_Mesh";

        target.add(branchMesh);
        let mat_id=1;
        for(let meshID of Object.keys(leavesContainer)){
            let mesh=this.meshes[meshID]["geo"];
            for(let matID of Object.keys(leavesContainer[meshID])){
                let mat=new THREE.MeshBasicMaterial();
                mat.side=THREE.DoubleSide;
                let count=leavesContainer[meshID][matID].length;
                let instancedLeaves=new THREE.InstancedMesh(mesh,mat,count);
        //handle material
                //let mat=this.meshes[neshID];
             let post_mat=this.materials[matID];
             target["materialLib"].push(mat);
                mat.roughness=0.4;
                ...
                instancedLeaves.castShadow=true;
                let waitAlphaMap=setInterval(()=>{
                  if(post_mat.alphaMap){
                      clearInterval(waitAlphaMap);
                      mat.alphaMap=post_mat.alphaMap;
                      mat.alphaTest=0.46;
                    instancedLeaves.customDepthMaterial = new THREE.MeshDepthMaterial( {
                        depthPacking: THREE.RGBADepthPacking,
                        alphaMap: post_mat.alphaMap,
                        alphaTest: 0.4
                    } );
                    instancedLeaves.customDepthMaterial.needsUpdate=true;
                ...
                let now_id=mat_id;
                let waitColorMap=setInterval(()=>{
                    if(post_mat.map){
                        clearInterval(waitColorMap);
                        mat.map=post_mat.map;
                        mat.needsUpdate=true;
                        target["materialLib"][now_id]["backMap"]=mat.map;
                    }
                ...
                instancedLeaves.instanceMatrix.needsUpdate = true;
                instancedLeaves.name="instancedLeaves_"+meshID+"_"+matID;
                for(let i=0;i<count;i++)instancedLeaves.setMatrixAt(i,leavesContainer[meshID][matID][i]);
                target.add(instancedLeaves);
                mat_id++;
            }
        }
        // transfrom to local frame
        let initBranch=this.branches[id];
        let last=initBranch.spinePoints.length-1;
       let offset=initBranch.posAbs;
        let firstPoint=new THREE.Vector3(initBranch.spinePoints.posX[0],initBranch.spinePoints.posY[0],initBranch.spinePoints.posZ[0]);
        let lastPoint=new THREE.Vector3(initBranch.spinePoints.posX[last],initBranch.spinePoints.posY[last],initBranch.spinePoints.posZ[last]);
        ...
        let nowAxis=lastPoint.addScaledVector(firstPoint,-1);
  
/*         let boxSize=boundingBox.getSize();
       let dem=Math.min(boxSize.x,boxSize.y);dem=Math.min(dem,boxSize.z);
        let scaleFactor=1/dem*5; */
        nowAxis.normalize();
        let targetAxis=new THREE.Vector3(0,1,0);
        let rotationAxis=new THREE.Vector3();
        rotationAxis.crossVectors(nowAxis,targetAxis);
        let rotationAngle=Math.acos(targetAxis.dot(nowAxis));
      /*   let scaleMat=new THREE.Matrix4();
        scaleMat.makeScale(scaleFactor,scaleFactor,scaleFactor); */
        target.rotateOnAxis(rotationAxis,rotationAngle);
       // target.applyMatrix4(scaleMat);
       // simplify material
...
function blendTree(t1,t2,blendingFactor){
    
   //blending tree
   let t1_current_level_id=t1.rootIndices;
	let t2_current_level_id=t2.rootIndices;
	let  turn = 0;
	let  current_level = 0;
	let maxLevel = Math.min(t1.maxLevel, t2.maxLevel);
	let ans=new TreeData();
	ans.maxLevel = maxLevel;
	ans.rootIndices =["1"];
    ans.maxID=[];
	while (current_level < maxLevel) {
		let t1_next_level_id=[];	let t2_next_level_id=[];
		let  now_idInlayer = 1;
		for (let  id  of t1_current_level_id) {
			let  now_branch_1 = t1.branches[id];
			if (!now_branch_1.isZeroBranch()) {
				 let itr= t2.branches[now_branch_1.id];
				 if (itr) {
					 let now_branch_2 =itr;
					 if (!now_branch_2.isZeroBranch()) {

						 let itr_p1 = ans.branches[now_branch_1.parentID];
						 let itr_p2 = ans.branches[now_branch_2.parentID];
						 if(itr_p1 || itr_p2 || current_level==0 ){
							 let  neo_branch = blendBranch(t1, t2, now_branch_1, now_branch_2);
							 neo_branch.layer = current_level;
							 neo_branch.computePart_Sum();
							 neo_branch.id = current_level + "_" +now_idInlayer;
							 // tackle relationship
							 if (current_level == 0)neo_branch.parentID = "-1_-1";
							 if (itr_p1&& itr_p2) {
								 if (turn % 2) {
									 neo_branch.parentID = itr_p1.id;
								
								 }
								 else {
									 neo_branch.parentID = itr_p2.id;
							 ...
							 if (neo_branch.parentID != "") {
								 if (current_level == 0) {
									 neo_branch.growingPos = -1;
								 }
								 else {
							 
									 neo_branch.posAbs = ans.branches[neo_branch.parentID].normalizedPosToWorld(neo_branch.growingPosOnParent, neo_branch.growingPos);
									 neo_branch.moveToWorld();
									 ans.branches[neo_branch.parentID].childrenBranch[neo_branch.growingPos]= neo_branch.id;
								 }
								 ans.branches[neo_branch.id] = neo_branch;
							/* 	 //blending Leaf
								 if (now_branch_1.childrenLeaves.size() != 0 && now_branch_2.childrenLeaves.size()) {
									 int len = std::min(now_branch_1.childrenLeaves.size(), now_branch_2.childrenLeaves.size());
									 for (int i = 0; i < len; i++) {
										 auto& l1 = t1.leavesInfo[now_branch_1.childrenLeaves[i]];
										 auto& l2 = t2.leavesInfo[now_branch_2.childrenLeaves[i]];
										 ans.leavesInfo.push_back(blendTwoLeaf(l1, l2, neo_branch));
										ans.branches[neo_branch.id].addChildLeaf(ans.leavesInfo.size() - 1);
									 }
		...
		//insert next level
		for (let  id of t1_current_level_id) {
			let cInfos = t1.branches[id].childrenBranchIndices;
			for (let cInfo of cInfos) {
				t1_next_level_id.push(cInfo);
			}
		}

        for (let  id of t2_current_level_id) {
			let cInfos = t2.branches[id].childrenBranchIndices;
			for (let cInfo of cInfos) {
				t2_next_level_id.push(cInfo);
			}
		}
		ans.maxID.push(now_idInlayer-1);
		t1_current_level_id = t1_next_level_id;
		t2_current_level_id = t1_next_level_id;
		current_level++;
	}
	//add meshes and materials
	ans.meshes = t1.meshes;
	ans.materialsInfo = t1.materialsInfo;
    ans.materials = t1.materials;


    let _group=ans.produceTreeGeo("tubeOnly",9,"1");
    return {data:ans,group:_group};
}
...
class BranchReferred{
  ...
  /*          if(id=="0_3_21"){
            this.material=new THREE.MeshStandardMaterial({color:"rgb(255,0,0)"});
          ...
          this.level=level;
           this.now_count=0;
           this.InstancedMesh.count=this.now_count;
           this.parentTreeID=[];
           this.matrix=[];
   }
   updateLOD(forest){
        this.InstancedMesh.count=0;
       let len=this.parentTreeID.length;
       let now_count=0;
      for(let i=0;i<len;i++){
             if(forest[this.parentTreeID[i][0]].lod>=this.parentTreeID[i][1]){
                this.InstancedMesh.setMatrixAt(now_count,this.matrix[i]);
                now_count++
             }


      }
       this.InstancedMesh.count=now_count;
...
/*
     let material=new THREE.MeshStandardMaterial({color:"rgb(127,127,127)"});
     let group=new THREE.Group();
     group.name="branchLib";
     for(let branch of Object.values(ans)){
        let geo=branch.geo;
        let mesh=new THREE.Mesh(geo,material);
        group.add(mesh);
     }
           ...
           let that=forest;
            ...
            setInterval(function(){
               ...
               forceRender();
                ...
                //requestAnimationFrame(that["updateLOD"]);
    };
    return forest;
}
        ...
        for(let tree of Object.values(this.treeLib)){
            ...
            for(let leavesLevel of Object.values(tree.leavesLibLOD) ){
                ...
                for(let treeID of Object.values(tree.InstanceID)){
                            ...
                            /*     tree.branchesLibLOD["2"].setMatrixAt(index,treeInstanced.transformMatrix);
                                tree.branchesLibLOD["2"].count++;
                                for(let level=3;level<=maxLevel;level++){
                                    tree.branchesLibLOD[String(level)].setMatrixAt(index,treeInstanced.transformMatrix);
                                 ...
                                 if(dist<1500){
                                    ...
                                    //add leaves
                           /*          for(let instancedMergedLeavesMesh of Object.values(tree.leavesLib)){ 
                                        instancedMergedLeavesMesh.setMatrixAt(index,treeInstanced.transformMatrix);
                                        instancedMergedLeavesMesh.count++;
                                    }; */
...
function setUpLeavesMaterial(mesh,material,species){
    
    mesh.material=material;
    mesh.material.alphaTest=0.46;
    mesh.material.roughness=0.4; 
    mesh.material.normalScale=new THREE.Vector2(0.6,0.6);
    let color_1="1.0, 1.0,1.0"; 
    let color_2="0.933, 0.811, 0.090";
    let hsvTarget="0.9,0.6,0.8"
    if(species=="Red_Maple"){
        hsvTarget="0.01,0.95,0.6";
        color_1="0.337, 0.862, 0.117";
        color_2="0.345, 0.694, 0.043";
    }else  if(species=="CypressOak"){
           hsvTarget="0.5,0.95,0.8";
            color_1="0.964, 0.721, 0.015";
            color_2="0.960, 0.505, 0";
    }else  if(species=="Black_Gum"){
        hsvTarget="0.01,0.99,0.7";
        color_1="0.964, 0.203, 0.015";
        color_2="0.972, 0.188, 0.070";
    }
    mesh.material.onBeforeCompile=function(shader,render){
          console.log("before leaves material compiling");
...
// Mix contributions from the five corners
  ...
  return 49.0 * ( dot(m0*m0, vec3( dot( p0, x0 ), dot( p1, x1 ), dot( p2, x2 )))
      ...
      float matTofloat(const in mat4 mat){
     float ans=0.0;
     ans+=mat[0][0]+mat[0][1]+mat[0][2]+mat[0][3];
     ans+=mat[1][0]+mat[1][1]+mat[1][2]+mat[1][3];
     ans+=mat[2][0]+mat[2][1]+mat[2][2]+mat[2][3];
     ans+=mat[3][0]+mat[3][1]+mat[3][2]+mat[3][3];
     return fract(ans);
     }
     varying  float instanceColorVariation;
     varying float v_u_time;
      `);
      shader.vertexShader=shader.vertexShader.replace('#include <fog_vertex>',
      `#include <fog_vertex>
      //add color variation per instance 
      float instancedInput=matTofloat(instanceMatrix);
      instanceColorVariation=instancedInput*instancedInput;
      float timeFactor=(mod(floor(u_time),15.0))*0.067;
      
      v_u_time=timeFactor;
      `);
      shader.vertexShader=shader.vertexShader.replace('#include <project_vertex>',
      `#include <project_vertex>
      //add vertex position change
      vec4 randomInput=vec4(mvPosition.xyz,u_time);
      mvPosition.y+=snoise(randomInput)*1.5;
      ...
      vec4 shiftHue(vec4 color,float hueAdjust){
...
vec3 hsv2rgb(const in vec3 c)
{
    ...
    return c.z * mix(K.xxx, clamp(p - K.xxx, 0.0, 1.0), c.y);
      ...
      vec3 color_hsv=rgb2hsv(diffuseColor.xyz);
      vec3 hsvTarget=vec3(${hsvTarget});
      //if(v_u_time>0.9)discard;
      //float timeFactor=v_u_time;
      //color_hsv=mix(color_hsv,hsvTarget,timeFactor*0.8+0.2*instanceColorVariation);
    
      diffuseColor=vec4(hsv2rgb(color_hsv),diffuseColor.a);
      `);
    ...
    mesh.customDepthMaterial.onBeforeCompile=function name(shader,render) {
        ...
        shader.vertexShader=shader.vertexShader.replace(
            ...
            //add vertex position change
           vec4 randomInput=vec4(mvPosition.xyz,u_time);
            mvPosition.y+=snoise(randomInput)*2.0;
            mvPosition.x-=snoise(randomInput)*2.0;
            gl_Position = projectionMatrix * mvPosition;
            float timeFactor=(mod(floor(u_time),15.0))*0.067;
            v_u_time=timeFactor;
            #include <logdepthbuf_vertex>
...
function setUpLeavesMaterial_unInstanced(mesh,species){
...
vec3 hsv2rgb(const in vec3 c)
{
    ...
    return c.z * mix(K.xxx, clamp(p - K.xxx, 0.0, 1.0), c.y);
      ...
      if(v_u_time>0.9)discard;
      float timeFactor=v_u_time;
      color_hsv=mix(color_hsv,hsvTarget,timeFactor);
