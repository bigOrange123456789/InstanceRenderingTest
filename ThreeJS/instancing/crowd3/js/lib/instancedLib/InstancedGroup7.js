function InstancedGroup(instanceCount,originMesh,animationClip,crowdData_json){
    ...
    this.widthAll=Math.floor(Math.sqrt(this.instanceCount/4));//输入着色器动画数据的宽度
...
InstancedGroup.prototype={
    ...
    initAnimation:function(uniforms,camera){
        ...
        uniforms.time={value: scope.time};
        uniforms.widthAll={value: scope.widthAll};
        uniforms.cameraX={value: camera.position.x};
        uniforms.cameraY={value: camera.position.y};
        uniforms.cameraZ={value: camera.position.z};
        ...
        function getTex(arr) {
            var addLength=arr.length%(4*scope.widthAll)
            console.log(
                'arr.length:'+arr.length,
                'line:'+4*scope.widthAll,
                'addLength:'+addLength)
            for(var i=0;i<addLength;i++)
                arr.push(0)
            var data = new Float32Array( arr.length);//1944
            var height = data.length/(4*scope.widthAll) ;//648
            data.set(arr)//for(var i=0;i<data.length;i++)data[i]=arr[i];//972
            var tex=new THREE.DataTexture(data, scope.widthAll, height, THREE.RGBAFormat,THREE.FloatType);
            return {"value":tex};
        }
    },
    init:function (texSrc,textNum,colors,texFlipY,finishFunction,camera){//纹理贴图资源路径，贴图中包含纹理的个数
        ...
        if(this.vertURL===undefined)this.vertURL=this.haveSkeleton?"shader/vertexBone7.vert":"shader/vertex.vert";
