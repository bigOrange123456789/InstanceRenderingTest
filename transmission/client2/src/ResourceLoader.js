...
class ResourceList{//这个对象主要负责资源列表的生成和管理
    ...
    */
    getModelFileInf1=function(opt){
        opt=opt||{};
        var n=opt.n===undefined?1:opt.n;

        var scope=this;
        if(scope.list.length===0)return null;
        if(n>scope.list.length)n=scope.list.length;

        var result=[];
        for(i=0;i<n;i++){
            result.push(scope.list[i]);
        }
        scope.list.splice(0,n);
        return result;
    }
    computeModelInf=function(i){
...
class ResourceList0{//这个对象主要负责资源列表的生成和管理
    maps;//所有贴图的说明信息
    models;//所有模型几何的说明信息
    mapsIndex;
    camera;
