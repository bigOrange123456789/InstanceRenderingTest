...
class ClothSimulate{
    ...
    applyObstacles(){
        ...
        const applyObstacle=( p1, obstacle )=> {
            ...
            const currentDist = diff.length();
            if ( currentDist >= obstacle.r ) return
            const correction = diff.multiplyScalar( (obstacle.r - currentDist) / currentDist )
            p1.position.add( correction )
        }
        
        for ( let i = 0; i < this.particles.length; i ++ ) {
            const particle = this.particles[ i ];
            applyObstacle( particle, {center:new THREE.Vector3(0,0,0),r:140*5} );
        }
    }
...
export class ClothMesh extends THREE.Mesh{
    constructor(){
        ...
        const sphere = new THREE.Mesh(
            ...
            new THREE.MeshBasicMaterial({ color: 0xf5f00 })
            )
        this.add(sphere)
        clothSim.obstacles.addMesh(sphere)
