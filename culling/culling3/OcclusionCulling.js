(function (exports) {
    ...
    function OcclusionCulling() {
        ...
        this.ndcRectIsOccluded = function (x0, x1, y0, y1, closestDepth) {
            //ndc矩形应该被剔除//应该被剔除返回true
            updateMipMaps();

            //x0, x1, y0, y1, closestDepth 的范围估计是-1~1//得到的是一个立方块
            ...
            for (var i = mipmaps.length - 1; i >= 0; i--) {
                ...
                var mipMapSize = Math.sqrt(mipmap.length); // TODO: Support non-square  支撑非方形
                ...
                if (maxx < 0 || maxy < 0 || minx >= mipMapSize || miny >= mipMapSize) {
                    return false;
                    // triangle is out of the screen. Can't determine if it's occluded. Should not cull!
                    //三角形不在屏幕上。无法确定是否被阻塞。不应该淘汰！
                }

                //减去图片上超出屏幕的部分
                //clamp(x, min, max)//x应该在取值范围之内
                ...
                for (var x = minx; x <= maxx; x++) {
                    for (var y = miny; y <= maxy; y++) {
                        ...
                        if (closestDepth < depth) {
                            // behind occluder.
                            // Rectangle is definitely occluded and we dont need to check next mipmap!
                            behindOccluder = false;//矩形是绝对闭塞，我们不需要检查下一个mipmap！
                        }
                    }
                }
                if (behindOccluder) {
            ...
            //对所有的mipmap检查了三角形，但没有被任何一个mipmap遮挡
            return false;
        }

        this.clear = function() {//每一帧都要进行一次清空
            blocks.forEach((b) => {
                b.clear();
            });
        };
        ...
        this.renderTriangles = function( indices, vertices, matrix ){
            ...
            for(var i=0; i<indices.length; i+=3){//循环体为对一个三角面的操作
                //以下是对三角面三个顶点的操作
                vectorSet(va, vertices[indices[i] * 3], vertices[indices[i]*3+1], vertices[indices[i]*3+2],1);
                vectorSet(vb, vertices[indices[i+1] * 3], vertices[indices[i+1]*3+1], vertices[indices[i+1]*3+2],1);
                vectorSet(vc, vertices[indices[i+2]*3], vertices[indices[i+2]*3+1], vertices[indices[i+2]*3+2],1);
                applyMatrix4ToVector4( va, matrix );//计算该顶点在屏幕上的位置
                applyMatrix4ToVector4( vb, matrix );
                applyMatrix4ToVector4( vc, matrix );
                vectorDivideScalar(va,va[3]); //divide by w
                vectorDivideScalar(vb,vb[3]);//除以w
