...
Terrain = function(options) {
    ...
    options.material = options.material || new THREE.MeshStandardMaterial({ color:"slate" });
    ...
    scene.terrainOptions=options;
    mesh.rotation.x = -0.5 * Math.PI;
    scene.add(mesh);

    scene.refreshHeightmap=function(canvas_id){
        let _mesh=this.children[0];
        let _options=this.terrainOptions;
        if(canvas_id){
            _options.heightMap= document.getElementById(canvas_id);
        }
        Terrain.fromHeightmap(_mesh.geometry.vertices, _options);
        Terrain.Normalize(_mesh, _options);
